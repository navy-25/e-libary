    ...
    <div class="card-header py-3 px-3">
        <div class="row">
            <div class="col-6 col-md-6 col-lg-6 d-flex">
                <p class="my-auto">Daftar Kata Kunci</p>
            </div>
            <div class="col-6 col-md-6 col-lg-6 d-flex">
                <a title="tambah" href="#"
                    onclick="create()"
                    ...
                    @foreach ($data as $number => $val)
                        <tr>
                            ...
                            <td style="width: 200px !important">
                                ...
                                <a href="{{ route('keyword.destroy',['id'=>$val->id]) }}" class="mb-1 btn btn-sm btn-danger">Delete</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
...
<script>
    ...
    function create(){
        $('#modal_title').text('Tambah Kata Kunci')
        $('#form').attr('action','{{ route('keyword.store') }}')
        $('#btn_submit').text('Tambah')
    }
    function update(id){
        var data = $(id).val()
        var data = JSON.parse(data)
        $('#modal_title').text('Edit Kata Kunci')
        $('#form').attr('action','{{ route('keyword.update') }}')
